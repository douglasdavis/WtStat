...
def run(args, prefixes):
    ...
    ax.errorbar(np.zeros_like(yval), np.array(yval) + Y_OFFSET_PT, xerr=xerr1,
                label=title1, fmt="ko", capsize=5 if args.just_one else 2)
    if not args.just_one:
        ax.errorbar(np.zeros_like(yval), np.array(yval) - Y_OFFSET_PT,
                    xerr=xerr2, label=title2, fmt="ro", capsize=2)
    for f1e, f2e, iyval in zip(xerr1, xerr2, yval):
        ...
        if not args.just_one:
            ax.text(-f2e - X_OFFSET_TEXT, iyval - Y_OFFSET_TEXT, "{}".format(round(f2e,3)),
                    color="red", horizontalalignment="right", verticalalignment="top", size=12)
    ax.set_xlim([-1.5, 1.5])
